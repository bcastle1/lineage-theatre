...
The private Blob store retains account/password-hash records, consent timestamps, rate limits, administrator invitations/activity, pricing settings, and opted-in cloud film metadata/videos. Any existing provider job or payment records are separate from these capabilities and do not prove current integration readiness. Original family sources and full working projects remain local under the signed-in account's project key. Cloud archive consent saves only whitelisted film details and a selected finished video with customer/administrator access; it does not silently upload source documents or the screenplay.
...
The Astra integration is prepared to read user-approved story text, source context, and selected reference photos, pending verified access. The active app has no alternative video provider or external MagicLight handoff. Highest quality is a preference until verified with the account's full-film API capabilities. MagicLight production and QuickBooks charges/refunds fail closed. Before enabling them, reserve durable per-user jobs, reconcile uncertain submissions without automatic duplicate charges, and verify actual output before marking a film complete. Customer payments and MagicLight expenses require separate auditable records.
