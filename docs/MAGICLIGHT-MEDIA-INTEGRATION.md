...
Development checkpoint: September 24, 2026 UTC. This branch adds native media processing, hosted payment authorization, an isolated MagicLight protocol client, and owner-only runtime diagnostics. The worker is staged and tested on the existing host. No authenticated MagicLight generation or live worker activation is claimed.
...
The initial public search found no generation documentation in MagicLight's sitemap or public OpenClaw navigation. [Magic Claw](https://magiclight.ai/claw/) is a waitlist/invitation-code product page. The authenticated Usage page showed the purchased 80,000 credits and no generation entries. Further developer-source research found concrete clients, described below. The requested technical follow-up was sent from Erik's connected mailbox to MagicLight support at `2026-09-24T03:44:12Z`, and the exact recipient, subject and body were verified in Sent Items. See [the inquiry record](MAGICLIGHT-API-REQUEST.md). No provider reply has been confirmed.

## Published client and private runtime checks

The versioned ClawHub client sources provide `Authorization: Bearer` authentication, `POST /api/misc/openclaw_add_task`, and `GET /api/misc/openclaw_check_task?task_id=...`. The production example uses `https://open.magiclight.ai`; two earlier examples use `https://open-test.magiclight.ai`. Both status endpoints returned HTTP 401 with `Authorization header required` when queried without credentials over verified HTTPS. This confirms reachable endpoints, not acceptance of the saved key. [Protocol evidence](MAGICLIGHT-PROTOCOL.md) records publisher limitations, versions, source hashes and exact request/response fields.

The isolated server client implements those requests with fixed origins, strict TLS, rejected redirects, bounded decoded response sizes and deadlines, secret-redacted errors, and no automatic submission retry. Submission is disabled by default. It never changes film readiness, uploads a reference image or retrieves a returned video. Compressed responses have a native-fetch regression test.

The owner-only Administration overview now includes **Check saved key**. Its same-origin, rate-limited POST uses the existing protected Vercel `MAGICLIGHT_API_KEY` inside the server and only checks a fresh nonexistent task ID. It accepts no key, host, text or job ID from the browser. It returns sanitized HTTP/business codes. HTTP 401 means authentication was rejected; a generic success or unknown-task reply remains unconfirmed until the provider's response meaning is established. A successful lookup does not turn on film production. This code must be deployed before the private check can be run with the existing protected secret; no new API key is requested.

Captured hosted orders also have an owner-only **Check recorded reversals** action. It exercises the real Accounting verifier through GET-only queries and an ordinary serialized OAuth refresh if necessary, with a 50-second overall deadline. It does not create a payment, refund, quote or production grant. A standalone CLI supports the same check; [reversal operations](HOSTED-REVERSALS.md) describes its precise scope. Production Vercel has the required saved configuration, but the current staged worker has no runtime credentials and these checks have not yet run against the real company.

The default full-film adapter remains unavailable because the examples do not establish exact costs or enforceable budgets, account quality/model options, request lookup after an uncertain submission, or complete reference/voice/output behavior. The existing IONOS host and offline FFmpeg acceptance are documented in [worker hosting](WORKER-HOST.md). Live readiness still requires an actual provider/account check, supported spending and media controls, live Accounting acceptance, worker secret provisioning and activation, and a bounded real render with private playback.
...
Local validation during this continuation passed **586 tests with zero failures or skips**, including actual FFmpeg decoding/encoding, hosted production authorization, recorded-reversal scans, provider-protocol fixtures, owner-only diagnostics, bounded responses, worker restart, authorization expiry before submission and customer readiness cases. The production TypeScript/Vite build passed with its existing large-bundle advisory. Independent review found and corrected a compressed-response length mismatch, an overly strong authentication interpretation, and a final diagnostic order-read race. The September 22 native-quality sample was independently decoded as 1920 by 1080, 24 fps, with AAC audio, and its stored byte length and SHA-256 were checked.
