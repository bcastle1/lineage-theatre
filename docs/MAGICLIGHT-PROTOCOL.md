...
## Private test-clip delivery

`magiclight-live-test-media.mjs` imports only the completed saved test, without calling the generation endpoint. Its exact host allowlist must be populated from the observed saved completion result; an empty list fails closed. No caller-selected URL, task ID or storage path is accepted. The media GET rejects redirects, omits credentials, enforces a 45-second download deadline and 100 MiB decoded-byte cap, and checks content type, declared length and MP4 header. This container check alone is not a full decoder or quality review.

The original bytes are uploaded create-only into private hash-addressed Blob storage, then fully read back and verified by size and SHA-256 within a separate deadline. A create-only metadata record binds the output to the original owner, test, fixture, task and source URL. Concurrent imports may recover the same verified copy; they never alter the permanent submission claim, customer payment, or film-production readiness.

Owner-only POST `importMagicLightLiveTestMedia` accepts only the action. GET/HEAD `magicLightLiveTestMedia` streams the verified private copy with byte ranges, strong ETag, safe download filename, and private/no-store headers. The Administration player and download use that fixed same-origin endpoint. An imported test clip is explicitly separate from the paid full film. Media access rechecks the persisted owner and completed claim, and does not require the original API key to remain active after import.

Full local decoding, measured duration/audio and signed-in playback are required before reporting a playable generation result. If the original reviewed media URL returns HTTP 401, 403, 404 or 410, import performs at most one status GET for that exact completed task and one download retry. The refresh must again report business code 10000, status 2 and a valid output URL; an echoed task ID must match. Both download destinations pass the exact host allowlist, omit credentials and reject redirects. Other download failures do not trigger source recovery. The download deadline includes recovery, and a late response cannot publish media.

The server-only refresh rechecks current owner, key and permanent claim before and after the provider request. It never changes the original completion record or issues another generation POST. The private sidecar retains its original source/task binding and separately records the SHA-256 of the URL actually downloaded. Signed URLs remain private; only their hashes enter that sidecar. Existing private copies are returned without another provider request and remain usable after provider-key rotation. A failed refresh preserves the completed evidence and requires a later explicit import retry.
