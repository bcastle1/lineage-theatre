...
test("canonical Intuit short invoice links are payable and recover an existing rejected invoice without another POST",async()=>{
  const cases=[["",""],["?locale=en_US","?locale=en_US"],["?locale=EN_us","?locale=EN_us"],
    ...
    ["?redirect=https%3A%2F%2Fevil.example%2FPRIVATE_QUERY_VALUE", ""],["?redirect=https://evil.example&locale=en_US","?locale=en_US"],
  ...
  for(const [suffix,expectedSuffix] of cases) {
    const base=`https://connect.intuit.com/t/scs-v1-${"aB09".repeat(24)}`,link=`${base}${suffix}`,destination=`${base}${expectedSuffix}`;
    const h=fixture({invoiceOverrides:{InvoiceLink:link}}),order=await h.checkout();
    assert.equal(order.status,"awaiting-payment");assert.equal(order.invoiceUrl,destination);assert.equal(order.charged,false);assert.equal(order.requiresReview,false);
    ...
    assert.equal(recovered.id,order.id);assert.equal(recovered.status,"awaiting-payment");assert.equal(recovered.invoiceUrl,destination);
    ...
    assert.equal((await h.checkout()).invoiceUrl,destination);assert.equal(mutationRequests(h).length,before);
